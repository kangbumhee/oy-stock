...
- 2026-09-13 후속 공개 범위: 저장소 자체는 비공개로 유지하며 gateway의 `search`/`options`만 이미지·상품명·옵션명과 필요한 옵션 식별자를 선별해 무료로 제공한다. 원본 증거·수집 내부 정보·매장·수량은 미리보기에 포함하지 않는다. `stores`는 계속 기기 인증과 유효 이용권을 확인하며, 권한 해제 시 브라우저의 재고만 지우고 공개 미리보기는 유지한다. 근처는 거리순, 전국은 수량 내림차순(동률 거리순, 미확인 수량 마지막)으로 표시한다.
