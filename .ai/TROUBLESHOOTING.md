...
### 온라인 재고만 보인 상태에서 주변 매장을 품절로 잘못 표시

- 발생일: 2026-09-13. 에스네이처 헬로키티 기획/투슬래시포 인 살몬 옵션에서 재현.
- 원인: 공식 `stock-stores`429와 서버 처리 지연 중 온라인 전용 빈 매장 목록을 재고 없음으로 렌더링했다. 후속 조회 실패가 짧은 알림만 남겨 온라인 우선 배너가 계속 남았고, 팝업/옵션 전환의 전국 선조회와 세션 재생성 재시도가 요청을 늘렸다.
- 해결: 근처/전국 공통 속도 제한·동일 요청 합치기·정상 응답만 캐시·429 대기 준수. 조회 중/일부 확인/조회 불가/실제 빈 결과를 구분한다. 전국 조회는 명시적인 클릭으로만 실행한다. 수동 재시도 대기, 선택 SKU 보존, 팝업을 닫은 뒤 늦게 도착한 응답 차단을 추가했다.
- 검증: 로컬 모의 브라우저390/1440과 Node 회귀 테스트. 수정 배포 전 에스네이처 실제 Cloud Run 응답에서 `ok`/재고 매장49/표시 행30을 확인해 상시 품절이 아닌 일시 실패임을 확인했다. 최종 운영 배포 이후 실측은 HANDOFF 참고.
- 관련 파일: `server/stock-request-runner.mjs`, `server/server.mjs`, `public/js/app.js`, `public/js/ui.js`, `tests/stock-detail-*`.
...
### 전국 매장 조회가 서울 마지막 페이지에서 멈춤

- 발생일: 2026-09-13
- 에러 메시지: `store_page_unavailable`.
- 원인: 공식 API는 마지막 다음 페이지에서 `SUCCESS`, `totalCount:0`, `storeList:[]`와 함께 `stockDisplayYn:false`를 반환한다. 해당 플래그만으로 오류로 판단하면 다음 시도로 넘어갈 수 없다.
- 해결법: 성공 응답·빈 배열·정확한0건이 함께 확인된 경우만 지역 종료로 인정한다. 표시 불가 플래그와 실제 행이 함께 있거나 총수가 불명확하면 계속 실패 처리한다. 짧은 페이지를 종료로 추정하지 않는다.
- 관련 파일: `server/hidden-store-pages.mjs`, `server/hidden-stock-backend.test.mjs`.
