...
- 숨김 옵션 클릭은 `App.lat/lng/locationName`의 선택 지역 근처부터 조회한다. 하단 전국 버튼을 눌러야 새 범위/커서로 전국 연속 조회한다. 목록/근처 화면 복귀, 기존 상품 팝업 보존, 페이지 갱신 시 스크롤/초점 보존을 추가했다.
...
- Node253개 테스트 및 Python Playwright360/390/1440폭 모의 브라우저 검증 통과. 실제 이미지 디코딩/대체 표시/명시적 전국 조회/중복 제거/복귀/권한 해제 검증, 페이지 오류0. 저장한 스크린샷 직접 확인.
