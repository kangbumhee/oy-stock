...
### 마지막 작업 — 2026-09-13 일반 근처 재고 조회 복구·온라인 품절 옵션 유료 조회 (배포 완료)

- 최신 요청은 두 상품의 오프라인 재고가 실제 조회되도록 수정하고 운영 배포하는 것이다. 작업은 기존 clean worktree `C:\Projects\oy-hidden-offline-20260913`에서 진행하며 원래 dirty workspace는 건드리지 않는다.
- 공식 매장 API429 및 Cloud Run 처리 지연과 온라인 우선 빈 매장 목록의 잘못된 품절 표시를 확인했다. 팝업/옵션 전환 시 전국 선조회 제거, 온라인 배경 배치 축소/팝업 중 일시정지/캐시 재사용, 근처·전국·유료 조회 공통 직렬 요청 큐와 정상 응답 캐시·대기 준수를 추가했다. 세션 재생성으로 요청 제한을 재시도하지 않는다.
- 프런트는 조회 중/미확인/부분 성공을 확정 품절과 구분하고 지속 오류 안내와 대기 후 재조회 버튼을 제공한다. 선택한 SKU/옵션번호 및 스크롤을 유지하고 팝업 닫기/다른 상품 전환 후 지연 응답을 차단한다. 기존 숨김 사진/이름/참고가격 및 유료 서버 권한은 유지한다. 정적 버전 `20260913-stock-recovery-1`.
- 수정 전 대기 후 실제 서버에서 에스네이처 `A000000263782`/SKU `8809506312905`가 HTTP200/`ok`/재고 매장49/반환30행으로 회복된 것을 확인했다. 이는 배포 후 수치가 아니며 최종 운영 확인을 아래에 추가해야 한다. 투슬래시포 `A000000227778`의 대상은 `[립칠러 미니 증정] 인 살몬`/SKU `8809923821608`이다.
- 로컬390/1440 모의 브라우저에서 미확인·재시도 성공·부분 실패·선택 옵션 유지·전국 명시 클릭·전국 수량순·팝업 경쟁을 통과했고 페이지 오류0/운영 요청0. 테스트 사진은 명시적 대체 사진이고 재고는 모의 값이다. Main이 모바일 실패/부분 실패 및 데스크톱 재조회 성공 이미지를 직접 확인했다. Node 최종 결과·소스/배포 식별자·실제 운영 화면 검증을 배포 후 기록한다.
- 남은 작업: 최종 회귀 → main 반영 → Cloud Run/Vercel production 배포 → 두 실제 상품의 매장 응답과 운영 UI 확인. 실제 제한이 계속되면 품절·복구 성공으로 주장하지 않는다.
- 진행: 소스 `0fca87f44910f391b27d66f798779d9eacdf9753`, Node318/318 통과. Cloud Run [34745591566](https://github.com/kangbumhee/oy-stock/actions/runs/34745591566) 성공, `oy-stock-api-00246-9kw` ready/트래픽100% 확인. 배포 후16:39 KST 투슬래시포7옵션 모두`ok`/각30매장, 선택된 인 살몬 SKU의 확인 수량은0이었다.
- 추가 사용자 요청: 온라인 품절된 투슬래시포도 유료회원에게 별도 오프라인 조회를 제공한다. 실제 재고를 본 매장은 모르며 있을 것으로 추측한 것이라고 답변했다. 온라인 품절 옵션에 명확한 유료 근처 조회 버튼을 매장 행 위에 표시하고 정확 SKU로 nearby→national 흐름을 제공한다. 일반 재고 조회의 기존 무료 범위를 새로 잠그는 변경은 아니다. 기존 숨김 목록은 온라인 미노출 옵션 발견이고 온라인에 노출된 품절 옵션과 구분한다.
- 실제 검증16:44 KST: 서비스 인증을 사용한 유료 재고 backend에서 `A000000227778`/`8809923821608` 근처200/132매장/양수0/미확인수량0/후속cursor 있음을 확인했다. 이는 첫 수집 묶음이며 전체 전국 품절을 뜻하지 않는다. 공식 사이트의 같은 옵션 김포 매장 조회도 미판매를 표시했다. 서비스 인증값은 출력/저장하지 않았고 운영 이용권 부여나 결제를 하지 않았다. 프런트 최종 버전은 추가 요청을 포함한 `20260913-stock-recovery-2`로 배포한다.
- 최종 소스 `ec2e1ba6ff69d2bee1b99be0809a2589b89857f9` main 반영. Node320/320, 기존 숨김 UI360/390/1440·일반 재고 및 온라인 품절 유료 진입390/1440 모의 브라우저 통과. Main이 온라인 품절 CTA/근처 유료 팝업 스크린샷을 직접 확인했다. 변경 정적6개 파일과 Vercel 사전 빌드 해시 일치.
- Vercel `dpl_9D3TubKm9byfE1RsNJT83Ki8hGoB` READY→promote 완료. [배포](https://oy-stock-ahazqb5bd-kbhs-projects-ee1427b6.vercel.app), [운영](https://olivestock.co.kr) inspect에서 같은 production/Ready 확인. Cloud Run은 위0fca87f4/00246-9kw이며 이후 UI-only 커밋에 서버 변경은 없다.
- 16:54~16:56 KST 운영 브라우저: config/ui/hidden-stock/app 모두`stock-recovery-2`. 투슬래시포 검색1개/정확 인 살몬 SKU 유지/새 유료 근처 CTA/무료 이용권 팝업 확인, 정상 근처 요청200과30매장 표. 버튼을 누르기 전 전국 선조회 없음. 무인증 유료 stores 요청401 `device_auth_required`/`private,no-store`/매장 데이터 없음. 실제 결제·이용권 부여는 하지 않았다.
- 같은 운영 브라우저의 김포 사우 기준 에스네이처 요청200, 근처48/50매장 재고·852개 요약/30행 표시(김포사우7/김포풍무1/김포장기15). 전국 버튼을 누른 후406/435매장·11912개 요약/100행 표시, 첫 수량668/439/379/219/187 및 전체 표시 행 내림차순 확인. 실제 제품 사진·매장표와 전국 정렬을 직접 렌더링 검토했다. 일반 전국은 기존10개 기준 지역 합집합/상위100행이며 모든 오프라인 매장 수집 완료가 아니다.
- 남은 이슈/경계: 조회는 복구됐지만 공식 요청 제한 자체를 제거하거나 재고 존재를 보장하지 않는다. 인 살몬은 확인한 첫132매장에 양수 재고가 없었으며 전국 전체 품절로 단정하지 않는다. 기존 신규 결제 팝업의 결제 준비 상태 확인 불가 경고는 유지돼 이번 작업에서 신규 결제 가능 여부는 확인하지 않았다. 유료권 있는 사용자의 화면은 모의 브라우저, 실제 매장 수집은 서비스 인증 backend로 각각 검증했다.

### 이전 작업 — 2026-09-13 매장 상품 카드 이름·참고가격
