...
### 마지막 작업 — 2026-09-13 매장 상품 카드 이름·참고가격

- 최신 사용자 요청은 사진 아래 기존 상품처럼 이름과 가격을 표시하고, 숨겨진 옵션 제목을 이해하기 쉽게 바꾼 뒤 운영 배포하는 것이다. 이전 이미지 전용 요청을 대체한다.
- 제목 `온라인에 없는 매장 상품`, 설명에는 온라인몰 미노출·과거 판매 옵션이며 현재 매장 재고는 별도 유료 조회라는 경계를 명시한다. 기존 카드 이미지·본문·상품명·굵은 가격 스타일을 재사용하며 옵션명도 별도 표시한다. 사진/상품명 클릭은 기존 이용권 안내 또는 유료 재고 팝업으로 연결한다.
- 가격을 새로 추정/수집하거나 유료 재고 API를 공개하지 않는다. 동일 goodsNo 안에서 정확 SKU/모순 없는 옵션번호의 기존 공개 priceToPay → `온라인 옵션 참고가`, 동일 상품 검색 priceToPay → `온라인 상품 참고가`, 기존 공개 detail.price → `최근 수집 참고가` 순이다. 다른 상품/옵션 가격이나 무효/0/음수는 쓰지 않으며 없으면 `매장 가격 확인 필요`를 표시한다. 실제 옵션·매장 가격과 다를 수 있음을 고지한다.
- 일반 검색/캐시/실시간 메타데이터 갱신 시 카드의 가격 부분만 갱신한다. 이미지/이름 버튼·초점·스크롤·유료 팝업은 재생성하지 않는다. 추가 가격 통신/새 Secret/Cloud Run 변경은 없다. 정적 버전 `20260913-store-product-cards-1`.
- Node282/282(API 권한24/UI37 포함), JS/SW 문법·diff 검사 통과. 브라우저360/390/1440에서 사진 아래 긴 상품명/옵션명/참고가 및 미확인 안내, 일반 카드와 이미지 폭·이름/가격 글꼴 일치를 확인했다. 늦게 도착한 정확 SKU 가격도 UI.updateCardBadge 경로에서 가격 부분만 갱신하며 초점/열린 유료 팝업/10매장을 보존했다. 기존 유료 권한·취소/승인·키보드·근처/전국 정렬 검증 유지, 페이지 오류0. Main이 최신 무료360/390/1440 스크린샷을 직접 확인했다.
- 소스 `4d709a5c55f285325100579eba280a82b430ddce` main 반영, 변경 정적5파일/gateway의 소스·사전 빌드 해시 일치. Vercel `dpl_DzjGdSiNevi14qdvNNM16MWWYCGG` READY 후 promote 완료. [배포 URL](https://oy-stock-p8h0lfrnw-kbhs-projects-ee1427b6.vercel.app), [운영 alias](https://olivestock.co.kr) inspect에서 같은 production/Ready 배포를 확인했다. Cloud Run·결제 설정 변경 없음.
- 16:05~16:06 KST 운영 `메디큐브` 검색에서 새 제목·설명 및5개 사진/상품명/옵션명/온라인 상품 참고가(33300/29900/24600/29900/23800원)를 확인했다. 모두1000px 실제 이미지, 새 JS/UI/CSS 버전 로딩, 렌더링 스크린샷 직접 확인. 두 번째 사진 클릭 시 `유료 이용자만 사용 가능합니다`, 닫기 후 원래 두 번째 사진 초점·5개 카드 유지·매장 행0 확인. 운영 결제 팝업은 기존 신규 결제 준비 상태 확인 불가 안내를 표시했으며 실제 결제·프로모션 부여·유료계정 조회는 수행하지 않았다. 유료 근처/전국 동작은 모의 API 회귀 범위다.

### 이전 작업 — 2026-09-13 숨김 옵션 이미지 전용 카드

- 최신 사용자 요청: 숨김 옵션을 기존 일반 상품과 같은 큰 정사각형 이미지 카드로 보여주고, 카드에는 사진만 표시한다. 무료 사용자의 이미지 클릭은 `유료 이용자만 사용 가능합니다` 이용권 팝업, 유효한 이용권 사용자의 클릭은 기존 근처 재고 팝업으로 연결한다. 검증 후 운영 배포를 요청받았다.
- 검색/상품별 옵션 목록은 기존 `.grid`/`.card` 크기·간격을 재사용한다. 카드에서 보이는 이름·설명·별도 재고 버튼을 제거하되 접근 가능한 버튼 이름에는 상품/옵션/유료 안내를 유지한다. 팝업 내부에는 선택 사진·상품명·옵션명을 표시한다. 정상 상품의 구매/제휴 링크 동작은 변경하지 않는다.
- accessOnly 이용권 팝업은 닫기 버튼으로 초점을 옮기고 Tab/Shift+Tab 순환·Escape 취소·원래 이미지로 초점 복귀를 처리한다. 기존 결제 처리 중 닫기 차단은 유지한다. 근처 거리순10개씩/맨 아래 전국 조회/전국 수량 내림차순·동률 거리순·미확인 마지막을 유지한다.
- 공개 API의 이름/옵션/이미지 허용 필드와 유료 매장 서버 권한은 변경하지 않는다. 이미지 전용은 화면 표현 정책이며 API에서 상품 식별 정보를 비밀로 만든다는 뜻이 아니다. Cloud Run/수집/결제 설정은 변경하지 않는다.
- CSS/hidden-stock/alerts/index/SW 캐시 버전 `20260913-hidden-cards-1`. 전체276개 Node 테스트 통과(API 권한24/UI31 포함), JS/SW 문법·diff 공백 검사 통과. 브라우저360/390/1440폭에서 실제 기존 상품 카드와 이미지 폭 일치(326/171/221px), 사진 디코딩·무료 결제 안내·모의 승인 재개·근처/전국 정렬·권한 제거를 확인했다. 두 번째 이미지의 Enter/Tab·Shift+Tab(정책 링크 포함)/Escape 복귀, 처리 중 Escape 차단, 중첩 팝업 복귀도 통과했다. Main이 최신 무료360/1440·유료 팝업360/1440 스크린샷을 직접 검토했다. 실제 결제/운영 유료권 부여 없이 모의 API로 유료 화면을 검증했다.
- 소스 `973444fc7dcc2c0c2af8527e3d90c6c98805b2b4` main 반영. 사전 빌드의 변경 정적5파일과 gateway가 검증 소스와 해시 일치. Vercel `dpl_4FFBsc5HQSumVvDw3bSL3LcgFGEy` READY 후 promote했으며 [배포 URL](https://oy-stock-i3vlnxyhy-kbhs-projects-ee1427b6.vercel.app), [운영 alias](https://olivestock.co.kr) inspect에서 같은 배포를 확인했다. Cloud Run은 변경하지 않았다.
- 운영 브라우저 확인(2026-09-13 14:47 KST): 새 JS/CSS `hidden-cards-1` 로딩, `메디큐브` 검색5개 카드 각각 실제1000px 이미지/표시 폭221px/보이는 텍스트 없음/기존 일반 상품 이미지와 폭 일치. 두 번째 카드 클릭→명시적인 유료 전용 제목/닫기 버튼 초점, Escape→같은 두 번째 이미지 초점 및5개 카드 유지, 매장 행0 확인. 무인증 전국 stores401 `device_auth_required`/`private, no-store, max-age=0`/stores 없음. 결제·프로모션 제출이나 운영 유료계정 재고 조회는 하지 않았다.

### 이전 작업 — 2026-09-13 옵션 무료 미리보기·재고 조회 결제·전국 수량순
