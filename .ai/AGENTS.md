...
- 2FA·휴대폰 인증·계정 잠금 해제·구매 승인은 자동화하지 않는다. 2026-09-06 사용자 요청에 따른 본인 계정 로그인용 CAPTCHA API 연동만 `scripts/lib/oy-captcha-*`에서 허용한다. 현재 폼/도전 검증·유료 작업 한도·명시적 API 활성화를 유지하며 다른 서비스/구매 작업으로 확대하지 않는다.
