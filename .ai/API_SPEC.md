...
- 운영자 메일: 활성화 시 complete/복구/webhook의 서버 검증과 이용권·결제의도 저장을 마친 뒤 승인·전액취소·부분취소 정보를 고정 수신자 `kbhjjan@gmail.com`으로 보낸다. 부분취소도 금액·통화·Store/Channel·LIVE·카카오페이 계약을 별도로 검증한다. 인증된 구매 이메일, 결제번호, 상품, 금액, 취소누계/잔액, 확인시각만 포함하고 인증키나 공급자 원문은 포함하지 않는다.
- 재시도: 암호화된 별도 outbox/영수증과 강한 ETag CAS로 승인 1건, 각 취소누계별 알림을 중복 방지한다. SMTP 실패는 대기열에 남겨 최대 5회 재시도하며 이용권 처리를 되돌리지 않는다. 대기열 저장 자체 실패만 `503 payment_notification_queue_unavailable`로 반환하여 PortOne 재전달을 요청한다. SMTP 접수 직후 저장 전 프로세스 종료 시 중복 가능성은 남으며 Message-ID는 고정한다.
- 범위: 활성화 기준시각 이후 확인된 거래만 발송한다. 다른 서비스의 PortOne 결제나 과거 결제 재전달에는 발송하지 않는다. 프로모션·수동 연장은 결제가 아니므로 대상이 아니다.
...
- 동작:
  - 활성 가격 알림이 없거나 취소로 이용권이 해제되어도 독립 결제 메일 outbox를 먼저 재시도한다. 1회 최대 20건/45초, 대기열 스캔 최대 1,000건이며 결과는 `ownerPaymentMail` 통계에 포함한다.
